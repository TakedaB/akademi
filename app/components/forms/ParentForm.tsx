...
type ParentChangeEvent = { target: { name: string; value: string } };

interface ParentFormProps {
  parent: ParentFormData;
  handleParentChange: (e: ParentChangeEvent) => void;
}
...
export default function ParentForm({
  ...
  const validadePhoneOnly = (value: string) => {
    return value.replace(/[^0-9+\-() ]/g, "");
  };

  const handlePaymentChangeInternal = (value: string) => {
    setPayment(value);
  ...
  const handleInputChangeWithValidation = (
    ...
    handleParentChange({ target: { name, value: sanitizedValue } });
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow p-0 border border-gray-200 overflow-hidden">
      ...
      <div className="p-6 grid grid-cols-2 gap-6">
        ...
        <div>
          <label className="block text-sm font-medium text-[#303972] mb-2"></label>
          <PaymentOptions
            selected={payment}
            onChange={handlePaymentChangeInternal}
          />
        </div>
      </div>
    </div>
